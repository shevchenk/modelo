...
var AddEditMenu=0; //0: Editar | 1: Agregar
var MenuG={id:0,menu:"",class_icono:"",estado:1}; // Datos Globales
...
$(document).ready(function() {
    ...
    $('#ModalMenu').on('shown.bs.modal', function (event) {
        ...
        $("#ModalMenu select").selectpicker('refresh');
        
        if( AddEditMenu==1 ){
            $(this).find('.modal-footer .btn-primary').text('Guardar').attr('onClick','AgregarEditarAjaxMenu();');
            $('#ModalMenuForm #txt_menu').focus();
        }
        else{
            $(this).find('.modal-footer .btn-primary').text('Actualizar').attr('onClick','AgregarEditarAjaxMenu();');
...
ValidaFormMenu=function(){
    var r=true;
    if( $.trim( $("#ModalMenuForm #txt_menu").val() )=='' ){
        r=false;
        msjG.mensaje('warning','Ingrese Menu',4000);
    }
    else if( $.trim( $("#ModalMenuForm #txt_class_icono").val() )=='' ){
        r=false;
        msjG.mensaje('warning','Ingrese Clase del Icono',4000);
    }
    return r;
}

AgregarEditarMenu=function(val,id){
    AddEditMenu=val;
    MenuG.id='';
    MenuG.menu='';
    MenuG.class_icono='';
...
CambiarEstadoMenu=function(estado,id){
    AjaxMenu.CambiarEstado(HTMLCambiarEstadoMenu,estado,id);
}

HTMLCambiarEstadoMenu=function(result){
    if( result.rst==1 ){
...
AgregarEditarAjaxMenu=function(){
    if( ValidaFormMenu() ){
        AjaxMenu.AgregarEditar(HTMLAgregarEditarMenu);
    }
}

HTMLAgregarEditarMenu=function(result){
    if( result.rst==1 ){
        msjG.mensaje('success',result.msj,4000);
...
HTMLCargarMenu=function(result){
    ...
    $.each(result.data.data,function(index,r){
        estadohtml='<span id="'+r.id+'" onClick="CambiarEstadoMenu(1,'+r.id+')" class="btn btn-danger">Inactivo</span>';
        if(r.estado==1){
            estadohtml='<span id="'+r.id+'" onClick="CambiarEstadoMenu(0,'+r.id+')" class="btn btn-success">Activo</span>';
        }
        ...
        html+="<input type='hidden' class='estado' value='"+r.estado+"'>"+estadohtml+"</td>"+
            '<td><a class="btn btn-primary btn-sm" onClick="AgregarEditarMenu(0,'+r.id+')"><i class="fa fa-edit fa-lg"></i> </a></td>';
        html+="</tr>";
    });
